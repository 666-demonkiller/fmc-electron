/*
Desgined and Developed by Sagar Pathere (TCET 2015-2019)
Packaged and Maintained by DemonKiller (TCET 2019-2023)
LICENSE: GPLv3
*/
...
function calculate(tbl,ip){
	...
	if(marks>=20 && marks<=100){
		...
		for(var i=0;i<5;i++){
			tbl.children[i].children[6-l[i] ].innerHTML='<img class="check" src="check.png">';
			w[i]*=l[i];
		}
		w.push(marks);
		return w;
...
function cell_check(event){
	...
	if(man_mode && td[0].tagName=="TD"){
		...
		tbl[0].children[row].children[col+1].innerHTML='<img class="check" src="check.png">';
		check_man();
	}
}
